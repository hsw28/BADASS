function f = decodeAcc_shuffle(clusters, acc, tdecode, pos, bin, dim, perms, varargin)
  % shuffles spike times and decodes acc based on shuffled cell firing.
...
% inputs =
            ...
            %position data
            %bin: = size of bins to bin acc into. we use 14cm^2/s per bin as default . this will always be symetric around zero
...
%ex: decodeAcc_shuffle(clusters, acc, .5, position, 14, 10, 100, 3.5);
...
for z = 1:perms
  ...
  [values errors] = decodeAcc(shuffclust, acc, tdecode, bin);
  [f pval rho]= spearman_rankresults(pos, acc, values, dim, pix_cm);
  rhovec(end+1)= rho;
